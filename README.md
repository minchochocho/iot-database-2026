...
- **설치 편의성** - 이미지만 있으면 컨테이너로 실행하는데 수십초에 불과. 설치설정 불필요
- **환경격차 문제 해결** - OS단의 설정까지 건드려야하는 문제를 없애고, 간단하게 서비스를 실행 가능 
- **서버비용 절감** - 새로운 서비스를 할 때마다 하드웨어 서버를 구매, 설정할 필요 없음
- **OS에 독립적** - 새로운 서비스의 운영OS에 따라 OS를 새소 설치할 필요없음
- **가상머신보다 빠름** - VMWare, VirtualBox와 같은 가상OS 플랫폼보다 실행속도가 빠름. 가상OS에서 필요없는 기능 제거 및 용량 축소
...
- DriverProperties - `allowPublicKeyRetrieval`을 `true`로 변경

![alt text](image-13.png)

### SELECT 실습

- 기본문법 - [쿼리](./Day02/1_SELECT기본.sql)
    ```sql
...
- WHERE 절 - 전체 데이터에서 필요한 것만 필터링
    - 비교 : `=`(같다), `<>`(같지 않다), `!=`(Db종류별로), `<`, `>`, `<=`, `>=`
    - 범위 : `BETWEEN` a `AND` b, `초과, 미만 사용불가` 날짜는 조심할 것
        - price `BETWEEN` 10000 `AND` 20000
    - 집합 : `IN`, `NOT IN`
        - price `IN`(10000, 20000, 250000) -- 가격이 1만, 2만, 2만5천에 속하는 데이터
        - price `NOT IN`(10000, 20000) -- 가격이 1만, 2만에 속하지 않는 데이터
    - 패턴 : `LIKE` (문자열만) , `%`, `_` 
        - bookname `LIKE` '축구%' -- 책제목중 축구로 시작하는 것 모두
    - NULL : 데이터가 없는 것, 입력되지 않은 것, `=`로 비교되지 않음! ~~where price = null~~
        - price `IS` NULL, price `IS NOT` NULL
    - 복합 : AND(C++ `&&`와 동일), OR(`||`), NOT(`!`)
...
- HAVING - 일반 필터링은 WHERE절로, `집계함수 필터링은 HAVING절`로

- GROUP BY, HAVING 주의사항
    - GROUP BY에 포함되지 않은 컬럼은 SELECT에 사용할 수 없음!
    - 집계한수 외 일반컬럼은 SELECT와 GROUP BY를 일치시킬 것!
    - HAVING 절에는 집계함수 필터링 포함
    - WHERE 절에 집계함수 사용불가!
    - SELECT, FROM, WHERE, GROUP BY, HAVING ORDER BY 순으로 기억

#### 조인 - [쿼리](./Day02/3_Join.sql)

- JOIN - 두 개 이상의 테이블을 합쳐서 하나의 테이블처럼 보여주는 기법

- JOIN 종류 - 종류는 많으나 3가지만 알면 됨
    - INNER JOIN(내부 조인) - 조인 중에서 가장 간단한 조인. 컬럼이 일치하는 데이터만 조회
    > 기준테이블X. 조인되는 테이블간의 관계를 확인

    - OUTER JOIN(외부 조인) - 한 테이블을 기준으로 데이터가 일치하지 않는 데이터까지 나오도록 조회하는 조인
        - LEFT OUTER JOIN - 두 개의 테이블 중 앞쪽 테이블을 기준
        - RIGHT OUTER JOIN - 두 개의 테이블 중 뒤쪽 테이블 기준

#### 서브쿼리(부속질의)

- SubQuery - 쿼리 내부에 포함되는 하위쿼리. 항상 소괄호 () 내에 작성 - [쿼리](./Day02/4_SUBQUERY.sql)
    - 서브쿼리는 소괄호 안의 쿼리부터 먼저 작성
    - 메인쿼리 - 소괄호 밖의 쿼리
    - 서브쿼리 - 소괄호 안의 쿼리
    - 대부분이 조인으로 변경 가능
    - 조인이 가지고 있는 성능개선의 특징을 사용할 수 없기에 속도저하가 발생할 가능성이 높음
...
- 서브쿼리 종류(쓰이는 위치에 따라) - [쿼리](./day03/1.SUBQUERY.sql)
    - WHERE절 서브쿼리
    - FROM절 서브쿼리
    - SELECT절 서브쿼리
...
- GROUP BY 컬럼 WITH ROLLUP - 전체 합산 추출 - [쿼리](./day03/3.ROOLUP.sql)
    - ROLLUP을 안쓰면 쿼리가 아주 길어짐


### DML 기타 - [쿼리](./day03/4.DML기타.sql)

- DML 중에서 직접적인 트랜잭션 영향을 받지 않는 것은 SELECT 뿐이다
...
- `DECIMAL(m, n)` - m(전체 65자리수), n(소수점 최대 30 자리수), 정수가 35자리, 소수점 30자리인 아주 큰 수
- DATE - 날짜만
- `DATETYPE` - 날짜와 시간 모두
- CHAR(n) - 고정길이 문자열 n만큼 길이 지정
    - CHAR(10)에 Hello를 입력하면 'Hello     '로 저장
    - 나머지 5자리 스페이스로 채움
    - ed) 주민번호, 공통코드처럼 정확한 길이 입력이 필요할 때
- VARCHAR - 가변길이 문자열 n만큼 길이 지정
    - VARCHAR(10)는 'Hello'로 저장. 나머지 5자리는 없앰
...
- 종류 : `기본키(Primary Key)`, 단일(Unique), 널허용여부(Null), 체크(Check), 기본값(Default), `외래키(Foriegn key)`
...
- CREATE 구문 - [쿼리](./day04/2.CREATE.sql)
    ...
    - FOREIGN KEY (custid) REFERENCES NewCustomer(custid) ON DELETE CASCADE
        ...
        - ON DELETE CASCADE : 무결성 유지를 위해서 부모테이블의 해당 PK데이터를 삭제하면 자식테이블의 관련 FK데이터도 같이 삭제하는 옵션
        - ON DELETE SET NULL : 부모테이블의 PK값이 삭제되면 자식테이블의 FK값은 NULL로 변경
        - ON UPDATE CASCADE | SET NULL : 수정할 때도 삭제시와 유사한 처리 가능. 수정도 가능하지만 PK수정이 거의 없기에 많이 사용되지 않음

- AUTO_INCREMENT : 테이블에 데이터를 삽입할 때 숫자타입 PK의 값을 자동 증가시켜서 만들어주는 기능
...
- VIEW - [쿼리](./day05/2.VIEW.sql)
    - 편리성과 재사용성 : 일반 테이블을 사용하는 것처럼 사용하고, 여러번 사용가능
...
- 뷰 특징
    ...
    - 뷰이지만 데이터 INSERT, UPDATE등이 가능
    - INSERT, UPDATE, DELETE는 거의 불가
    - 뷰는 보기 위해서 생성하므로 SELECT 이외의 DML은 거의 사용하지 않음
...
- 인덱스 종료
    ...
    - UNIQUE 인덱스 : UNIQUE 제약조건의 컬럼에 걸 수 있는 인덱스, NULL허용 데이터 중복X
...
- Common Table Expression : 공통으로 쓸 수 있는 테이블 표현기법 - [쿼리](./day05/4.CTE.sql)
    - 여러곳에서 공통으로 사용할 임시 테이블형태의 쿼리
    - 이름을 지정하는 임시 테이블
    - 쿼리가 존재하는 동안만 있는 가상(임시)테이블
    - 쿼리를 깔끔하게 생성
    - 쿼리 실행동안 재사용
    - 가상데이터를 생성할 때
...
- Entity Relationship Diagram
    - 개체 관계 다이어그램 : 관계형 DB에 사용된 테이블의 상호관계를 그림으로 구조화한 것 
    - 세상의 사물을 개체(Entitiy)와 개체 간의 관계(Edlationship)으로 표현
- ERD 모델링 툴
    - ERWin Data Modeler : 퀘스트 사에서 만든 대표적인 ERD 작성툴. 업계표준. 유료
    - eXERD : 한국사 모델링 툴, 이클립스 기반
    - ER/Wtudio : 대규모 엔터프라이즈 데이터 모델링툴. 유료
    - Draw.io - https://app.diagrams.net/ 개념, 논리 ERD 작성 가능. 무료
    - `ERDCloude` - https://www.erdcloud.com/ 한국에서 개발한 웹기반 모델링 툴. 논리/물리 ERD 작성, 내보내기. 광고제거 유료
    - DBeaver - 물리 ERD 뷰어 제공. 모델링 불가
    - MySQL Workbench - DBMS 관리툴. 물리 ERD 작성 가능. MySQL DB 생성 장점
...
- 학원 수강관리 시스템 - [쿼리1](./day08/1.ERD_DATABASE_USER_생성쿼리.sql), [쿼리2](./day08/1.ERD_TABLE_생성쿼리.sql)
...
- 수강관리 시스템화 되기 이전 문제점
    - 이상현상
        - 삽입 이상 : 수강생이 없는 새 과목은 추가가 어려움(과목, 강사명, 강사전화만 입력해면 의미있는 데이터X)
...
- **제1졍규형**(1NF)
    ...
    - 전화번호 컬럼에 010-1111-1111, 010-2222-2222 두개의 값이 들어가면 안됨
...
- **제3정규형**(3NF)
    ...
    - 과목정보, 강사정보로 분리. `이행적 (함수) 종속성 제거`를 해야 함
    - 강사정보를 구분지을 수 있는 키 속성이 생성되어야 함
...
- MySQL 데이터베이스화
    ...
    - 학원이름(부경 IT 아카데미, PKIT Academy)
    - 학원 수강신청 시스템(Institude Enrollment System) -> PA-IES
...
- **BCNF(Boyce-Cdd 정규형)**
    ...
    - 키 종류 : 기본키, 외래키, 복합키(두 개의 키), 슈퍼키
    - 후보키 : 슈퍼키(기본키)가 될 수 있는 속성들
    - 슈퍼키 : 행을 유일하게 식별할 수 있는 속성 집합(한 개 또는 여러 개). 유일하기만 하면 됨
    ...
    - (학생, 과목) -> 강사 결정가능
    - 과목만 가지고도 강사 결정가능. 학생은 강사를 지정할 키가 아님
...
- **제 4정규형**($NF)
    - `다중값 종속 제거`
    - 처음부터 설계가 잘못된 것이 대부분. ERD 작성시 발생경우가 거의 없음
...
- **제 5정규형**(5NF)
    - `조인 종속 제거`
    - 설계로 테이블을 나눠도 조인을 하면 원래 데이터가 나와야 함
    ...
    - 단, 쿼리 실행 시 속도저하가 발생가능 -> 반정규화
    - 빠른게 장땡

### 대량 데이터 인덱스 실습
    - 100만건 이상의 데이터에서 인덱스를 제대로 설정하지 않으면 조회쿼리 시 속도 저하 발생
...
- 실행계획(Excution Plan) 확인 - [쿼리](./day08/6.인덱스_실습쿼리.sql)
    - DB 쿼리 최적화 분석 방법

- 실행계획 결과
```sql
-> Sort: ob.order_date DESC  (cost=1.04e+6 rows=9.96e+6) (actual time=2581..2581 rows=26 loops=1)
    -> Filter: (ob.customer_id = 123456)  (cost=1.04e+6 rows=9.96e+6) (actual time=1131..2580 rows=26 loops=1)
        -> Table scan on ob  (cost=1.04e+6 rows=9.96e+6) (actual time=0.0689..2289 rows=10e+6 loops=1)
```

- 위 내용 분석 : 제일 아래 -> 부터 분석
    1. Table scan on orders_big : cost비용(메모리를 쓰는 비용), rows(Table sacn으로 읽는 행수, 995만 정도), 0.0745초로 속도가 느리지 않음
    2. Filter : customer_id를 123456로 필터링, actual time이 증가. 1차적 문제
    3. Sort : order_date 내림차순 정렬, actual time 3282..3282이 최종 처리시간, 
    - where절에서 필터링 되는 값은 인덱스를 걸어주는게 좋다

- 인덱스 추가 후
```sql
-> Index lookup on ob using idx_orders_customer_id_and_order_date (customer_id=123456) (reverse)  (cost=26.8 rows=26) (actual time=0.463..1.4 rows=26 loops=1)
```
...
- 개발방법
    - MySQL 8.0이상 추천 (8.0.45)
    - MySQL Sever 자체 라이브러리 사용
    - Visual Studio 프로젝트 생성
    - C++ 코드 작성
...
- 기본 연결확인 구현
- 테이블 데이터 확인
    - 쿼리문 문자열 마기막 ; 무조건 제거(오류발생)

- MySQL 연동 순서 - [소스](./day09/MySqlMadangApp/main.cpp)
    1. 콘솔 인코딩 UTF-8 설정
    2. 연결, 행데이터, 결과 구조체 포인터 변수 선언
    3. MySQL 초기화
    4. 접속정보로 접속
    5. 서버 문자셋 UTF-8로 설정
    6. 쿼리 실행
    7. 결과 메모리 저장
    8. 한 행씩 Fetch 및 출력(SELECT에 한함)
    9. 결과 메모리 해제
    10. 접속 종료

#### MySQL CRUD 앱 구현

- Book 테이블 CRUD 테스트 - [소스](./day09/MysqlCrudTest/MysqlCrudTest.cpp)

- C학습 AddressBook 프로젝트와 비교
    - 텍스트파일, File IO vs MySQL DB
    - contact 구조체 vs MySQL 자체 구조체
...
- MySQL C API 함수목록
    ...
    - mysql_free-result() - 쿼리실행결과 메모리 해제
    - mysql_affected_row() - 쿼리실행 처리 행 수 리턴
    - mysql_close() - DB연결 종료\
